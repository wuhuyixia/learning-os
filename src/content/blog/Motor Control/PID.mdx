...
import PIDDemo from "../../../components/PIDDemo.astro";
...
拖动下面的 **Kp / Ki / Kd** 滑块，可以实时观察闭环阶跃响应的变化。蓝色虚线表示设定值（Setpoint），红色曲线表示系统实际响应（Response）。

<PIDDemo />

可以重点观察以下规律：

- **Kp（比例）** 增大时，系统通常响应更快，但过大时更容易产生超调和振荡。
- **Ki（积分）** 可以逐步消除稳态误差，但过大时会增加超调并使系统更难稳定。
- **Kd（微分）** 对误差变化趋势进行预测，可提供阻尼、抑制振荡，但过大时会对噪声更敏感。

> 演示中的被控对象是简化二阶机械模型，主要用于直观理解 PID 参数变化趋势。实际电机中的惯量、阻尼、负载、采样周期、限幅和滤波都会影响最终参数。
